...
export default async function boot(...args) {
	...
	if (options.imports) {
		if (!specifier) {
			throw new Error('The boot sequences needs to be loaded from a path if imports is defined!');
		}

		importMap = registerCustomImports(options.imports, specifier[1]);
	}

	if (options.beforeLoading) options.beforeLoading(app);

	if (specifier && specifier[0] != null) {
		if (importMap) specifier[0] = resolveImportMap(specifier[0], importMap);

		if (specifier[0].startsWith('#')) {
			specifier[0] = resolveImportMap(specifier[0], options.resolve?.importMap || createImportMapGetter(specifier[1])());
		}

		if (specifier[0].startsWith('.')) {
			specifier[0] = `${new URL(specifier[0], specifier[1])}`;
		}

		sequences = await import(specifier[0]);
	}
