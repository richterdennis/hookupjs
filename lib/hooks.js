...
function resolveImportMap(specifier, parentURL) {
	const pkgPath = module.findPackageJSON(parentURL);
	...
	if (specifier.startsWith('.')) {
		const parentDir = path.dirname(parentURL);
		const pkgDir = path.dirname(pathToFileURL(pkgPath));
		const relativePath = path.posix.relative(parentDir, pkgDir) || '.';
