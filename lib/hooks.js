...
export function registerResolvers(options) {
	...
	function resolve(specifier, context, nextResolve) {
		...
		if (
			...
			// Don't internal resolve for require() because it resolves directories and extensions by default
...
function internalResolve(specifier, parentURL, options) {
	...
	if (specifier.startsWith('#')) specifier = resolveImportMap(specifier, parentURL);
...
function resolveImportMap(specifier, parentURL) {
	// package.json path starts with \\?\ on windows which results in an error using require
	const pkgPath = module.findPackageJSON(parentURL).replace('\\\\?\\', '');

	// Hopefully package.json getting cached using require
	const pkg = module.createRequire(parentURL)(pkgPath);

	if (!pkg.imports) return specifier;

	for (let [search, replace] of Object.entries(pkg.imports)) {
	...
	// The relative path defined in the imports field is relative to the package.json
	if (specifier.startsWith('.')) {
		const parentPath = path.dirname(fileURLToPath(parentURL));
		const relativePath = path.relative(parentPath, path.dirname(pkgPath)) || '.';

		specifier = `${relativePath}/${specifier}`.replaceAll(path.sep, '/');
	}
